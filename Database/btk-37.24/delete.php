...
<body>
    ...
    */
    ...
    $Sil  = $VeriTabaniBaglantisi->prepare("DELETE FROM uyeler WHERE id=?");
    $Sil->execute([10]);
    */
     $Sil  = $VeriTabaniBaglantisi->prepare("DELETE FROM uyeler WHERE id=10");
     $Sil->execute();
        if($Sil){
            echo "Silme işlemi başarılı.<br/>";
        }else{
