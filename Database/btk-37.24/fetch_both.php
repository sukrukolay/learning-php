...
<body>
    ...
    */
    ...
    $Sorgu  = $VeriTabaniBaglantisi->prepare("SELECT *FROM uyeler",array(PDO::FETCH_BOTH)); // sorgu prepare() sayesinde hazırlandı.
    $Sorgu->execute(); // sorgu execute() ile çalıştırıldı.
        if($Sorgu){
            $KayitSayisi    = $Sorgu->rowCount();
                if($KayitSayisi>0){
                    foreach($Sorgu as $Satirlar){
                         echo $Satirlar[0]." | ".$Satirlar[1]." | ".$Satirlar[2]." | ".$Satirlar[3]." <br/> ";
                        // aşağıdaki satırda aynı sonucu verir. FETCH_BOTH yapısında iki kullanımda doğrudur.
